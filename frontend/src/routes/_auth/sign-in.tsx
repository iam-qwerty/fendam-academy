...
function SignInPage() {
  ...
  const { verified } = useSearch({ from: '/_auth/sign-in' })
  ...
  const isVerifiedRedirect = verified === '1'

  useEffect(() => {
    if (session) {
      const role = getUserRole(session.user)
      const hasStudentProfile = !!(session.user as Record<string, unknown>)
        ?.studentProfile
      ...
      {isVerifiedRedirect && (
        <div className="rounded-2xl border border-primary/20 bg-primary/10 px-4 py-3 text-sm text-primary">
          <div className="flex items-center gap-2">
            ...
            <span>
              Your email is verified. Sign in to continue your onboarding.
            </span>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {verificationEmail && (
          ...
          >
            ...
            {!resending && (
              <HugeiconsIcon icon={ReloadIcon} className="ml-2 h-5 w-5" />
            )}
          </Button>
        )}
      </form>
