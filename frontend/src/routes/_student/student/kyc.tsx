...
export const Route = createFileRoute('/_student/student/kyc')({
  component: StudentKycPage,
})
...
function StudentKycPage() {
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      const idCardResult = await apiFetch<{
        signedUrl: string
        fileKey: string
      }>('/uploads/presigned', {
        method: 'POST',
      ...
      const paymentResult = await apiFetch<{
        signedUrl: string
        fileKey: string
      }>('/uploads/presigned', {
        method: 'POST',
  ...
  const isSubmitted = ['submitted', 'under_review', 'approved'].includes(
    kycStatus.kycStatus,
  )
            ...
            <div>
              ...
              {kycStatus.latestRecord && (
                <p className="text-sm text-muted-foreground">
                  Submitted{' '}
                  {new Date(
                    kycStatus.latestRecord.createdAt,
                  ).toLocaleDateString()}
                </p>
              )}
            </div>
          ...
          {isRejected && kycStatus.latestRecord?.reviewComment && (
            <div className="mt-4 rounded-lg border border-destructive/20 bg-destructive/5 p-4 text-sm">
              ...
              <p className="mt-1 text-muted-foreground">
                {kycStatus.latestRecord.reviewComment}
              </p>
            </div>
          )}
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <Button
              ...
              disabled={
                submitMutation.isPending || !idCardFile || !paymentFile
              }
              className="w-full"
            >
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground">
              Your documents are being reviewed by an admin. You will be notified
      ...
      {kycStatus.kycStatus === 'approved' && (
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground">
              Your identity has been verified. You have full access to the
              platform.
            </p>
          </CardContent>
        </Card>
