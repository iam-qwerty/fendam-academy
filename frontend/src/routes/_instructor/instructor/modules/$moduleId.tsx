...
export const Route = createFileRoute('/_instructor/instructor/modules/$moduleId')({
  component: InstructorModuleDetailPage,
})
...
function InstructorModuleDetailPage() {
  ...
  const { data: assignmentsResponse } = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ data: Assignment[] }>(
        `/instructor/assignments?moduleId=${moduleId}`,
      ),
    staleTime: 5 * 60 * 1000,
    enabled: !!moduleId,
  })
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate({ to: '/instructor/modules' })}
        >
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {module.track.name} &bull; Module #{module.orderIndex}
          </p>
        </div>
      </div>
